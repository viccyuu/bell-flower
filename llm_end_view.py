...
"""
LLM End Viewer
将 llm-log.db 中的 llm_end 数据导出为 HTML 报告
"""
...
def fetch_llm_end_records(conn: sqlite3.Connection, limit: int = 100) -> list[dict]:
    """获取 llm_end 数据"""
    query = """
    SELECT 
        id,
        ...
        event,
        ctx,
        json_extract(ctx, '$.sessionId') as session_id,
        json_extract(ctx, '$.sessionKey') as session_key,
        json_extract(ctx, '$.agentId') as agent_id
    FROM llm_end
...
def fetch_session_ids(conn: sqlite3.Connection) -> list[str]:
    """获取所有唯一的 session_id"""
    query = """
    SELECT DISTINCT json_extract(ctx, '$.sessionId') as session_id
    FROM llm_end
    WHERE json_extract(ctx, '$.sessionId') IS NOT NULL
    ORDER BY json_extract(ctx, '$.sessionId') ASC
    """
    cursor = conn.execute(query)
    return [row['session_id'] for row in cursor.fetchall() if row['session_id']]


def generate_html(records: list[dict], session_ids: list[str], output_path: str) -> None:
    """生成 HTML 报告"""
...
<body>
        ...
        len(records)) + """</div>
            ...
            <div class="control-group">
                ...
                <div id="timeRange" style="font-size: 13px; color: #666;">
                    """ + (records[-1]['created_at'][:16] if records else 'N/A') + """ 至 """ + (
               records[0]['created_at'][:16] if records else 'N/A') + """
                </div>
            </div>
        </div>

        <!-- Content -->
        <div class="section-title">📋 Agent End Events</div>
        <div id="topLevelList" class="top-level-list"></div>
        <div id="content"></div>
    </div>
    ...
    <script>
        ...
        function filterBySession() {
            ...
            if (sessionId) {
                filteredRecords = allRecords.filter(r => r.session_id === sessionId);
            } else {
        ...
        function renderTopLevelList() {
            const container = document.getElementById('topLevelList');
            ...
            // 获取第一个记录的 event
            const firstEvent = JSON.parse(filteredRecords[0]['event']);
            const topLevelKeys = Object.keys(firstEvent);

            // 生成顶层元素按钮
            container.innerHTML = topLevelKeys.map(key => 
                '<div class="top-level-item" onclick="showTopLevelContent(\\'' + key + '\\')">' + key + '</div>'
            ).join('');

            // 默认显示第一个元素
            if (topLevelKeys.length > 0) {
                showTopLevelContent(topLevelKeys[0]);
            }
        }

        // 显示顶层元素内容
        function showTopLevelContent(topLevelKey) {
            const contentContainer = document.getElementById('content');

            // 更新按钮样式
            document.querySelectorAll('#topLevelList .top-level-item').forEach(item => {
            ...
            const records = filteredRecords.map((r, index) => {
                const event = JSON.parse(r['event']);
                return {
                    index: index + 1,
            ...
            records.forEach(record => {
                const value = record.event[topLevelKey];
                const isSuccess = record.event.success !== false;
                ...
                html += '<span>📁 Session: ' + record.sessionId + '</span>';
                html += '<span>🤖 Agent: ' + (record.agentId || 'N/A') + '</span>';
                html += '<span>🕐 ' + record.time + '</span>';
                html += '<span class="' + (isSuccess ? 'status-success' : 'status-error') + '">' + (isSuccess ? '✅ Success' : '❌ Error') + '</span>';
                html += '</div></div>';

                // 使用树形展示
                html += '<div class="tree">';
                html += renderTree(value, topLevelKey);
                html += '</div>';

                // 显示错误信息（如果有）
                if (!isSuccess && record.event.error) {
                    html += '<div style="margin-top: 15px; padding: 15px; background: #f8d7da; border-radius: 5px; border-left: 4px solid #dc3545;">';
                    html += '<strong style="color: #721c24;">⚠️ Error:</strong> ';
                    html += '<span style="color: #721c24;">' + record.event.error + '</span>';
                    html += '</div>';
                }
...
def main():
    ...
    # 输出路径
    output_path = r"D:\PycharmProjects\PythonProject\output\llm-end-report.html"
    ...
    session_ids = fetch_session_ids(conn)
    print(f"✅ 获取到 {len(session_ids)} 个 Session")

    # 获取数据
    print("🔍 查询 llm_end 数据...")
    records = fetch_llm_end_records(conn, limit=100)
    print(f"✅ 获取到 {len(records)} 条记录")

    # 统计成功/失败
    success_count = 12
    # sum(1 for r in records if json.loads(r['event']).get('success', True) \
    #                     if isinstance(r['event'], str): else: r['event'].get('success', True))

    # 生成 HTML
    print("📝 生成 HTML 报告...")
    generate_html(records, session_ids, output_path)
